  ...
  export namespace poisson {
    ...
    export function quantile(p: f64, lambda: f64): f64 {
      ...
      */
      ...
      if (u <= 0.0 || v <= 0.0) return NaN;

      let d: f64, s: f64, x = 0.0, ilam = 1.0 / lambda;

      if (lambda > 4.0) {
        let w = normal.quantile(Math.min(u, v));
        if (u > v) w = -w;
        ...
        if (Math.abs(w) < 3) {
          ...
          s = w / Math.sqrt(lambda);
          let r = Math.max(1.0 + s, 0.1);
          let t: f64, r2: f64, s2: f64;

          do {
            t = Math.log(r);
            r2 = r;
            s2 = Math.sqrt(2.0 * (1 - r + r * t));
            if (r < 1.0) s2 = -s2;

            r = r2 - (s2 - s) * s2 / t;
            if (r < 0.1 * r2) r = 0.1 * r2;

          } while (Math.abs(r - r2) > 1e-8);

          t  = Math.log(r);
          s  = lambda * r + Math.log(Math.sqrt(2.0 * r * (1 - r + r * t)) / Math.abs(r - 1)) / t;
          s -= 0.0218 / (s + 0.065 * lambda);
          d  = 0.01 / s;
          s += d;
        }
        ...
        if (s > 10.0 && s < x + 2.0 * d) {
          ...
          if (x > 0.5 * lambda && x < 2.0 * lambda) {
            ...
            s /= 1.0 + b1 * xi;
            s *= Math.exp(-0.5 * x * eta * eta) / Math.sqrt(2.0 * Math.PI * x);
            if (x < lambda) {
              s += 0.5 * erfc_approx(eta * Math.sqrt(0.5 * x));
              if (s > u) x -= 1.0;
            } else {
              s -= 0.5 * erfc_approx(-eta * Math.sqrt(0.5 * x));
              if (s > -v) s -= 1.0;
            ...
            s =  (1.0 /   12.0) + t;
            s *= xi;
            s = (x - lambda) - x * Math.log(x * ilam) - s;

            if (x < lambda) {
              let t = Math.exp(-0.5 * s);
              s = 1.0 - u * t * t * Math.sqrt(2.0 * Math.PI * xi) * lambda;
              t = 1.0;
              xi = x;
              for (let i = 1; i < 50; i++) {
                xi -= 1.0;
              ...
              t = Math.exp(-0.5 * s);
              s = 1.0 - v * t * t * Math.sqrt(2.0 * Math.PI * x);
              xi = x;
              for (let i = 0; i < 50; i++) {
                xi += 1.0;
                s = s * xi * ilam + 1.0;
